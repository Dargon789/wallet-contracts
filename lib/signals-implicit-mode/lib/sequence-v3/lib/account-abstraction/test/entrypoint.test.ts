...
describe('EntryPoint', function () {
  ...
  describe('Stake Management', () => {
    ...
    describe('without stake', () => {
      ...
      it('should fail to stake without delay', async () => {
        await expect(entryPoint.callStatic.addStake(0, { value: ONE_ETH })).to.revertedWith(
  ...
  describe('flickering account validation', () => {
    it('should prevent leakage of basefee', async function () {
      ...
      const maliciousAccount = await new MaliciousAccount__factory(ethersSigner).deploy(
        entryPoint.address,
        { value: parseEther('1') }
      )
      ...
      try {
        ...
        await ethers.provider.send('evm_mine', [])
        await expect(
          simulateValidation(userOpPacked, entryPoint.address, { gasLimit: 1e6 })
    ...
    it('should limit revert reason length before emitting it', async () => {
      ...
      const testRevertAccount = await new TestRevertAccount__factory(ethersSigner).deploy(
        entryPoint.address,
        { value: parseEther('1') }
      )
    ...
    describe('warm/cold storage detection in simulation vs execution', () => {
      ...
      it('should prevent detection through getAggregator()', async () => {
        const testWarmColdAccount = await new TestWarmColdAccount__factory(ethersSigner).deploy(
          entryPoint.address,
          { value: parseEther('1') }
        )
        const badOp: UserOperation = {
        ...
        try {
          ...
          if ((e as Error).message.includes('ValidationResult')) {
            const tx = await entryPoint.handleOps([badOpPacked], beneficiaryAddress, {
              gasLimit: 1e6,
            })
            await tx.wait()
          } else {
      ...
      it('should prevent detection through paymaster.code.length', async () => {
        ...
        const paymaster = await new TestPaymasterAcceptAll__factory(ethersSigner).deploy(
          entryPoint.address
        )
  ...
  describe('2d nonces', () => {
    ...
    it('should fail nonce with new key and seq!=0', async () => {
      ...
      await expect(entryPoint.callStatic.handleOps([op], beneficiaryAddress)).to.revertedWith(
    ...
    describe('with key=1, seq=1', () => {
      ...
      it('should allow manual nonce increment', async () => {
        ...
        const incNonceKey = 5
        const incrementCallData = entryPoint.interface.encodeFunctionData('incrementNonce', [
          incNonceKey,
        ])
  ...
  describe('without paymaster (account pays in eth)', () => {
    describe('#handleOps', () => {
      ...
      it('should revert on signature failure', async () => {
        ...
        const beneficiaryAddress = createAddress()
        await expect(entryPoint.estimateGas.handleOps([op], beneficiaryAddress)).to.revertedWith(
      ...
      describe('should pay prefund and revert account if prefund is not enough', function () {
        ...
        before(async () => {
          ...
          paymaster = await new TestPaymasterWithPostOp__factory(ethersSigner).deploy(
            entryPoint.address
          )
        ...
        it('without paymaster', async function () {
          ...
          const rcpt = await entryPoint
            .handleOps([packUserOp(await createUserOpWithGas(vgl - 1, 0, minCallGas))], beneficiary)
            .then(async (r) => r.wait())
        ...
        it('with paymaster', async function () {
          ...
          const minPmVerGas = await findUserOpWithMin(
            async (pmVgl: number) => createUserOpWithGas(minVerGas, pmVgl, minCallGas),
            false,
            entryPoint,
            1,
            100000,
          ...
          const rcpt = await entryPoint
            .handleOps(
              [packUserOp(await createUserOpWithGas(minVerGas, minPmVerGas - 1, minCallGas))],
              beneficiary
            )
            .then(async (r) => r.wait())
            .catch((e: Error) => {
      ...
      it('should fail to call recursively into handleOps', async () => {
        ...
        const callHandleOps = entryPoint.interface.encodeFunctionData('handleOps', [
          [],
          beneficiaryAddress,
        ])
      ...
      it('should report failure on insufficient verificationGas after creation', async () => {
        ...
        await expect(simulateValidation(op1, entryPoint.address)).to.revertedWith('AA23 reverted')
      })
    })
    ...
    describe('aggregation tests', () => {
      ...
      it('should fail to execute aggregated account without an aggregator', async () => {
        ...
        await expect(entryPoint.handleOps([userOp], beneficiaryAddress)).to.revertedWith(
          'AA24 signature error'
        )
      })
      ...
      it('should reject non-contract (address(1)) aggregator', async () => {
        ...
        const aggAccount1 = await new TestAggregatedAccount__factory(ethersSigner).deploy(
          entryPoint.address,
          address1
        )
    ...
    describe('with paymaster (account with no eth)', () => {
      ...
      before(async () => {
        paymaster = await new TestPaymasterAcceptAll__factory(ethersSigner).deploy(
          entryPoint.address
        )
      ...
      it('should not revert when paymaster reverts with custom error on postOp', async function () {
        ...
        const errorPostOp = await new TestPaymasterRevertCustomError__factory(ethersSigner).deploy(
          entryPoint.address
        )
    ...
    describe('Validation time-range', () => {
      ...
      describe('validatePaymasterUserOp with deadline', function () {
        ...
        before('init account with session key', async function () {
          this.timeout(20000)
          paymaster = await new TestExpirePaymaster__factory(ethersSigner).deploy(
            entryPoint.address
          )
      ...
      describe('handleOps should abort on time-range', () => {
        it('should revert on expired account', async () => {
          ...
          await expect(entryPoint.handleOps([userOp], beneficiary)).to.revertedWith(
