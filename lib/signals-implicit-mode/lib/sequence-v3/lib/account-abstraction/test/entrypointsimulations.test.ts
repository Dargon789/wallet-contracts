...
*/
  ...
  describe('#simulateValidation', () => {
    ...
    it('should revert on oog if not enough verificationGas', async () => {
      ...
      await expect(simulateValidation(op, entryPoint.address)).to.revertedWith('AA23 reverted')
    })
    ...
    it('should report failure on insufficient verificationGas (OOG) for creation', async () => {
      ...
      await expect(simulateValidation(op1, entryPoint.address, { gas: '0xF4240' })).to.revertedWith(
  ...
  describe('over-validation test', () => {
    ...
    before(async () => {
      ...
      await simulateValidation(packUserOp(userOp), entryPoint.address).catch((e) => {
  ...
  describe('#simulateHandleOp', () => {
    it('should simulate creation', async () => {
      const accountOwner1 = createAccountOwner()
      const factory = await new SimpleAccountFactory__factory(ethersSigner).deploy(
        entryPoint.address
      )
