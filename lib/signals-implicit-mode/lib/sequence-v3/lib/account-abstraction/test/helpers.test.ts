...
describe('#ValidationData helpers', function () {
  ...
  it('#parseValidationData', async () => {
    ...
    expect(
      await helpers.parseValidationData(
        packValidationData({ aggregator: AddressZero, validUntil: 0, validAfter: 10 })
      )
    ).to.eql({ aggregator: AddressZero, validAfter: 10, validUntil: max48 })
    expect(
      await helpers.parseValidationData(
        packValidationData({ aggregator: AddressZero, validUntil: 10, validAfter: 0 })
      )
  ...
  it('#parseValidationData (typescript)', async () => {
    ...
    expect(
      ...
      parseValidationData(packValidationData({ aggregator: addr, validUntil: 10, validAfter: 0 }))
    )
  })
  ...
  it('#packValidationData', async () => {
    ...
    expect(hexlify(await helpers.packValidationData(true, 123, 456))).to.eql(
      hexlify(packValidationData({ aggregator: addr1, validUntil: 123, validAfter: 456 }))
    )
  })
