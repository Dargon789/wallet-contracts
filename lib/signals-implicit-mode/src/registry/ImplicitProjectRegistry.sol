...
contract ImplicitProjectRegistry is IImplicitProjectRegistry {
  ...
  function addProjectUrlBatch(
    bytes32 projectId,
    string[] memory projectUrls
  ) external onlyProjectOwner(projectId) {
  ...
  function _removeProjectUrlHash(
    bytes32 projectId,
    bytes32 projectUrlHash,
    uint256 urlIdx
  ) internal {
