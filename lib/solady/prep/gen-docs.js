...
async function main() {
  ...
  const getTopIntro = s => {
    // First, locate the @notice block up to and including its author/parenthesized terminator.
    const noticeBlockRegex =
      /\/\/\/\s+@notice\s+[\s\S]*?(?:\/\/\/\s?@author[^\n]*\n|\/\/\/\s+\([^\n]*\)\n)/;
    const noticeMatch = s.match(noticeBlockRegex);
    if (!noticeMatch) return null;

    // From the end of the @notice block, capture everything up to the first contract/library declaration.
    const remainder = s.slice(noticeMatch.index + noticeMatch[0].length);
    const bodyRegex = /([\s\S]*?)(?:library|abstract\s+contract|contract)\s[^.]+\{/;
    const bodyMatch = remainder.match(bodyRegex);
    if (!bodyMatch) return null;

    const intro = bodyMatch[1];
    return normalizeNewlines(strip(
      intro.replace('\n\n', '\n\n\n').split('\n')
      .map(l => l
